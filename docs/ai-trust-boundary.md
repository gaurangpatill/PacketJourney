...
Authoritative references do not broaden this capability. The application—not the model—selects a fixed versioned index, namespace, embedding model, and metadata filter. Vector matches must resolve through D1 and the checked-in manifest. Source text is a separately delimited untrusted quotation and cannot add tools or instructions.
...
8. Validation that every technical citation ID was supplied and that every frozen URL/publisher/category/source identity matches the enabled manifest.
...
The validated response contains a conclusion type (`supported`, `likely`, `inconclusive`, or `unsupported`), confidence, summary, answer, evidence-linked findings and actions, explicit uncertainties, evidence references, technical-reference claims, follow-up questions, and bounded graph instructions. Server-controlled metadata adds the question, generation time, model/prompt, frozen validated citations, retrieval versions/status, and whether a deterministic fixture produced the response.
...
Persisted retrieval provenance contains a sanitized controlled query and question hash, not raw prompts or full question history. D1 stores frozen excerpts and hashes but not embeddings or complete source documents beyond normalized bounded chunks. Saved reference cards are historical snapshots, not claims that the source is still current.
