...
AI is not part of the Layer 7 simulation engine. Suggestions are deterministic, users explicitly run a strict scenario, and fixed rules calculate topology and metric effects before model interaction. The optional explanation surface receives the completed simulated investigation plus a bounded projection of its exact change and assumption IDs through the existing diagnosis endpoint. Simulation-specific claims must cite those IDs and the validator rejects invented provenance. The model may summarize cited simulated evidence or say evidence is insufficient; it cannot select a scenario, invoke a rule, supply a simulated number, change assumptions, or convert an unavailable metric into an estimate. AI unavailability never blocks simulation or export.
